<div class="px-3 py-5 mb-3 shadow-sm hover:shadow-md duration-200 rounded border border-gray-200">
    <div class="flex items-center justify-between">
        <a href="{{ route('jobs.show', $job->id) }}" class="text-xl font-bold text-green-800">{{ $job->title }}</a>
    ...
    <a href="{{ route('jobs.show', $job->id) }}" class="ml-1 text-gray-800 font-semibold hover:text-green-800">{{ __('job.consult_mission') }}</a>
    </div>
    ...
    {{ __('job.open') }} • {{ number_format($job->price / 100, 2, ',', ' ') }} €
    </div>
    <span class="block text-sm">{{ __('job.they are') }} <span  class="text-indigo-400">{{ $job->proposals->count() }}</span> {{ trans_choice(_('job.how much proposition'), $job->proposals) }}</span>
</div>
